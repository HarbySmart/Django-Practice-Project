...
def summary(request):
    ...
    interactions = Interactions.objects.filter(interaction_date_gte=thirty_days_ago)
...
def interact(request,cid):
    ...
    if request.method == "POST":
        ...
        channels = request.POST["channel"]
        directions = request.POST["direction"]
